window.LD = (function(){
  ...
  var events = {
    request_login: [
      function(account_id){ LD.account_id = account_id; }
    ],
    request_logout: [
      function(){ LD.account_id = null; }
    ]
  };
  ...
  LD.request_login = function(){
    window.parent.postMessage(JSON.stringify({name: 'request_login', args: []}), '*');
  };

  LD.request_width = function(new_width){
    window.parent.postMessage(JSON.stringify({name: 'request_width', args: [new_width]}), '*');
  };

  LD.request_height = function(new_height){
    window.parent.postMessage(JSON.stringify({name: 'request_height', args: [new_height]}), '*');
  };

  LD.request_redirect = function(path) {
    window.parent.postMessage(JSON.stringify({name: 'request_redirect', args: [path]}), '*');
  }

  LD.request_z_index = function(selector) {
    window.parent.postMessage(JSON.stringify({name: 'request_z_index', args: [selector]}), '*');
  }
